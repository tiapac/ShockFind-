...
def add_a_conditioned_grid(sel_cond         = True,
    ...
    dx = 800./512.
    points             = np.column_stack((x1*dx-400, y1*dx-400, z1*dx-400))       
...
if __name__ == "__main__":
    ...
    poss=np.array((plotter.generate_orbital_path(factor = 1.1*box, n_points=NR_FRAMES+1, viewup=(0,0,1), shift=box/2)).points, dtype = float)
    plotter.camera.position=poss[0]
    for ii,pos in enumerate(poss):
        
        add_a_conditioned_grid(sel_cond = shocks[6]==12, title = "FS", color = "blue",posy = size*2+size//10, unicolor = "blue" ,inistate = True, add_vector_field=False)
        add_a_conditioned_grid(sel_cond = shocks[6]==34, title = "SS", color = "red", posy = size*3+size//10, unicolor = "red"  ,inistate = True, add_vector_field=False)
        pos = 4
        if 0:
            for i in range(7,15):
        ...
        if 0: plotter.add_slider_widget(
        ...
        plotter.screenshot("faculty_day/shocks_%05d"%ii)
